...
use gng_core::{packet::PacketWriter, PacketFileData};
...
fn create_packet_meta_data_directory(
    ...
    writer.add_path(&mut gng_core::packet::Path::new_directory(
...
fn side_facets_from(
    ...
    packet: &gng_core::PacketFileData,
...
pub struct Facet {
    pub packet_name: gng_core::Name,
    pub packet_version: gng_core::Version,
    pub facet_name: gng_core::Name,

    pub facet_definition_packet: gng_core::Hash,

    pub mime_types: Vec<String>,
    pub patterns: Vec<glob::Pattern>,
    pub contents_policy: crate::ContentsPolicy,

    writer: Option<Box<dyn gng_core::packet::PacketWriter>>,
}
...
impl Facet {
    ...
    pub fn store_path(
        ...
        package_path: &mut gng_core::packet::Path,
        _mime_type: &str,
    ) -> eyre::Result<()> {
        tracing::info!(
            "\"{}\": Storing {:?}.",
    ...
    pub fn finish(
        &mut self,
    ) -> eyre::Result<Option<(gng_core::Name, std::path::PathBuf, gng_core::Hash)>> {
    ...
    fn write_facet_metadata(&mut self) -> eyre::Result<gng_core::Name> {
        ...
        let data = gng_core::FacetFileDataBuilder::default()
            .packet_name(packet_name)
            .name(facet_name.clone())
            .version(packet_version)
            .build()?;

        let buffer = serde_json::to_vec(&data).map_err(|e| gng_core::Error::Conversion {
        ...
        writer
            .add_path(&mut gng_core::packet::Path::new_file_from_buffer(
                buffer,
                &meta_data_directory,
...
pub struct MainFacet {
    pub packet: gng_core::PacketFileData,

    pub contents_policy: crate::ContentsPolicy,

    pub writer: Option<Box<dyn gng_core::packet::PacketWriter>>,
}
...
impl MainFacet {
    ...
    pub fn finish(
        &mut self,
        facets: &[(gng_core::Name, gng_core::Hash)],
        factory: &super::InternalPacketWriterFactory,
    ) -> eyre::Result<Option<(PacketFileData, std::path::PathBuf, gng_core::Hash)>> {
        check_contents_policy(
            self.writer.is_some(),
            &self.contents_policy,
            &self.full_debug_name(),
    ...
    fn write_facet_metadata(
        &mut self,
        facets: &[(gng_core::Name, gng_core::Hash)],
    ) -> eyre::Result<gng_core::PacketFileData> {
